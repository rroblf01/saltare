...
> **Status: 1.3.0 — lazy TLS, eight new Zig-side or launcher knobs, leanest of three.** Production target is **Linux x86_64**. v1.3 ships orthogonal wins: (1) **OpenSSL link is gone** — `tls.zig` `dlopen`s libssl on the first `serve(ssl_certfile=...)` call. (2) **Health endpoint intercept** (`health_path="/healthz"`) — k8s probes get `200 OK\nok\n` from Zig. (3) **CORS preflight intercept** (`cors_preflight_allow_all=True`) — OPTIONS-with-Origin requests skip Python and get permissive CORS headers. (4) **IPv6 listen** — `host="::"` / `[::1]` auto-detects v6, binds `AF_INET6` with `IPV6_V6ONLY=1`. (5) **`mallopt(M_ARENA_MAX, 1)` at module init** plus **`MALLOC_ARENA_MAX=1` injected into the `saltare` CLI re-exec environment** so the very first CPython allocations also land in a single arena. (6) **Per-IP rate limiter** (`rate_limit_per_sec`, `rate_limit_burst`) — token bucket in Zig, 4096-IP bounded LRU table, over-rate gets 429 before the user app sees it. **Honors `proxy_headers`**: rate-limits by the leftmost `X-Forwarded-For` address rather than the proxy's TCP peer when that flag is on. (7) **`tracemalloc_path`** (e.g. `"/debug/tracemalloc"`) — auto-starts `tracemalloc.start(25)` and serves a top-30 Python allocation dump. (8) **`PYTHONOPTIMIZE=2` auto re-exec** — `saltare` CLI re-execs itself under `python -OO` (unless `SALTARE_NO_OPTIMIZE=1` is set), strict-gated to only fire when this module is the actual main entry (importing `saltare.cli` from a third-party script no longer surprises anyone). Plus **URL decoding moved to Zig** (`http.urlDecode`), so `_dispatcher.py` no longer imports `urllib.parse`, and **`TCP_NODELAY` set on every accepted socket** so small-response latency drops the Nagle delay. Combined: saltare is the leanest of the three benchmarked ASGI servers — **45.3 / 44.3 / 44.0 MiB across the three workloads**, vs uvicorn 47.8 / 48.4 / 53.3 MiB and Granian 55.9–56.2 MiB on the same host (fair comparison: each launcher imports the FastAPI app at module level so master RSS reflects the same import footprint). Tests **66 passing** (10 new in `tests/test_v13.py` cover health / CORS / rate-limit / tracemalloc / IPv6 / URL decode).
...
|---------|-----------|----------------|-----------|---------|------|
| saltare | 45.16 MiB |      45.29 MiB | **45.30 MiB** |    1000 | 1132 |
| uvicorn | 47.80 MiB |      47.84 MiB | 47.84 MiB |    1000 | 1277 |
| granian | 56.21 MiB |      56.21 MiB | 56.21 MiB |    1000 | 1185 |

### Concurrent — 100 clients × 20 requests (2000 total)

| server  | idle RSS  | RSS after load | peak RSS  | reqs ok | rps  |
|---------|-----------|----------------|-----------|---------|------|
| saltare | 43.91 MiB |      44.24 MiB | **44.26 MiB** |    2000 | 1947 |
| uvicorn | 47.48 MiB |      48.39 MiB | 48.39 MiB |    2000 | 1927 |
| granian | 56.06 MiB |      56.06 MiB | 56.06 MiB |    2000 | 1910 |

### Idle keep-alive — 500 connections held open

| server  | idle RSS  | RSS after load | peak RSS  | reqs ok | conn rate |
|---------|-----------|----------------|-----------|---------|-----------|
| saltare | 43.82 MiB |      44.00 MiB | **44.00 MiB** |     500 | 1463      |
| uvicorn | 47.97 MiB |      53.34 MiB | 53.34 MiB |     500 | 1405      |
| granian | 55.88 MiB |      55.88 MiB | 55.88 MiB |     500 | 1290      |
...
|       4 |        4 |  14.19 MiB |     39.30 MiB | 53.50 MiB |  156.58 MiB (−66%) |
...
> **saltare is the leanest of the three on every workload.** v1.3's lazy-loaded TLS, the `mallopt` arena cap (with `MALLOC_ARENA_MAX=1` injected into the CLI re-exec env), the gated `PYTHONOPTIMIZE=2` re-exec, and `TCP_NODELAY` together dropped the floor by ~3-4 MiB vs v1.2.2. The seven Zig-side / launcher knobs (health intercept, CORS preflight, IPv6, rate limiter, tracemalloc endpoint, metrics endpoint, URL decode in Zig) ship at zero RAM cost (no extra mmaps, no extra heap unless turned on). Saltare leads granian by 10.9–11.9 MiB and uvicorn by 2.5–9.3 MiB. Throughput on this run: saltare beats both peers on the concurrent workload, trails uvicorn slightly on sequential — saltare optimises for RAM, not raw rps. Where saltare's *architectural* advantage shows most: idle-keepalive 500 conns adds **+0.18 MiB** to saltare (~370 B/conn) vs uvicorn's **+5.37 MiB** (~11 KiB/conn) and granian's **0.00 MiB** — saltare and granian are both effectively free on idle-conn growth; only uvicorn pays per-connection cost.
...
- **Per-connection slope vs uvicorn**: 500 idle connections cost saltare **+0.18 MiB** (~370 B/conn) vs uvicorn's **+5.37 MiB** (~11 KiB/conn). That's a **~30× per-connection memory saving** vs uvicorn for a realistic workload (clients that hold connections open between bursts of activity). Granian adds essentially nothing (`+0.00 MiB`), so on per-conn slope saltare and granian are both comparable; saltare leads on the floor and on uvicorn-vs-anyone.
...
- [x] **v1.3.0** — Lazy-loaded TLS, eight new operational knobs, leanest of three. RAM cuts: (1) **OpenSSL link gone at build time** — `tls.zig` declares OpenSSL types as `opaque {}`, hard-codes ABI constants, and ships a function-pointer table populated by `dlopen` + `dlsym` on first `newContext()` call. Plain-HTTP deployments never load libssl/libcrypto. (2) **`mallopt(M_ARENA_MAX, 1)` at module init** caps glibc's per-thread arenas. The `saltare` CLI re-exec also injects **`MALLOC_ARENA_MAX=1`** into the child env so even CPython's bootstrap allocations land in a single arena. (3) **`PYTHONOPTIMIZE=2` auto re-exec** strips docstrings + asserts from FastAPI / Pydantic / Starlette — `SALTARE_NO_OPTIMIZE=1` opts out, and `_is_saltare_main_entry()` gates the re-exec to only fire when this module is the actual main entry (importing `saltare.cli` from a third-party script no longer surprises anyone). (4) **URL decode moved to Zig** (`http.urlDecode`) — `_dispatcher.py` no longer imports `urllib.parse`. (5) **`TCP_NODELAY` set on every accepted socket** — small-response latency loses the Nagle delay. New operational knobs (all opt-in, zero RAM cost when off): (6) **Health endpoint intercept** (`health_path="/healthz"`) — Zig answers `200 OK\nok\n`; k8s probes never touch Python. (7) **CORS preflight intercept** (`cors_preflight_allow_all=True`) — OPTIONS-with-Origin gets permissive CORS headers from Zig. (8) **IPv6 listen** — `host="::"` / `[::1]` auto-detects v6, binds `AF_INET6` with `IPV6_V6ONLY=1`. (9) **Per-IP rate limiter** (`rate_limit_per_sec`, `rate_limit_burst`) — token bucket in Zig, 4096-IP bounded LRU table, returns 429 before Python sees the request. **Honors `proxy_headers`**: rate-limits by leftmost `X-Forwarded-For` when that flag is on, so requests behind a trusted proxy aren't all bucketed under the proxy's TCP IP. (10) **`tracemalloc_path`** (e.g. `"/debug/tracemalloc"`) — auto-`tracemalloc.start(25)` and serves a top-30 dump for leak hunts in long-running deployments. Bench: **45.3 / 44.3 / 44.0 MiB** vs uvicorn **47.8 / 48.4 / 53.3 MiB** and granian **56.0–56.2 MiB** on the same host (fair comparison: `benchmarks/run_granian.py` imports the FastAPI app at module level so granian's master RSS reflects the same import footprint saltare carries). Tests **66 passing** (10 new in `tests/test_v13.py`).
