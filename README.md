...
> **Status: 1.3.0 — lazy TLS + a dozen Zig-side / dispatcher knobs, leanest of three.** Production target is **Linux x86_64**. v1.3 ships orthogonal wins: (1) **OpenSSL link gone** — `tls.zig` `dlopen`s libssl on first `serve(ssl_certfile=...)` call. (2) **Health intercept** (`health_path`). (3) **CORS preflight intercept** (`cors_preflight_allow_all`). (4) **IPv6 listen** (auto-detect from `host`). (5) **`mallopt(M_ARENA_MAX,1)` at module init** + `MALLOC_ARENA_MAX=1` in the CLI re-exec env. (6) **Per-IP rate limiter** (`rate_limit_per_sec`, `rate_limit_burst`) — honors `proxy_headers` (rate-limits by `X-Forwarded-For` leftmost when behind a trusted proxy). (7) **`tracemalloc_path`** auto-starts tracking + serves top-30 dump. (8) **`PYTHONOPTIMIZE=2` auto re-exec** (gated to only fire on `saltare` CLI / `python -m saltare`, and adds `MALLOC_ARENA_MAX=1` to the child env). (9) **URL decoding moved to Zig** — drops `urllib.parse` import. (10) **`TCP_NODELAY`** + **`SO_KEEPALIVE`** on every accepted socket. (11) **`favicon_204`** — Zig answers `GET /favicon.ico` with 204 (browsers spam it on every page). (12) **`max_connections_per_ip`** — TCP-level RST on over-cap peers (shares the rate-limit table). (13) **`access_log_path`** routes JSON access-log lines to a file instead of stderr. (14) **`request_id_header`** auto-generates an 8-byte hex ID per request, exposes via `scope["x-request-id"]` and echoes as a response header. (15) **`server_timing=True`** emits `Server-Timing: total;dur=<ms>` on every response. (16) **WebSocket subprotocol** finally honored — `Sec-WebSocket-Protocol` parsed from the request into `scope["subprotocols"]`, app's `accept(subprotocol=...)` echoed in the 101 response (was a real bug). (17) **HTTP trailers** (`http.response.trailers`) emitted via chunked-encoding trailer block. Combined: saltare is the leanest of the three benchmarked ASGI servers — **47.0 / 45.8 / 45.6 MiB**, vs uvicorn 49.1 / 49.8 / 54.6 MiB and Granian 56.6–57.7 MiB on the same host (fair comparison: each launcher imports the FastAPI app at module level so master RSS reflects the same import footprint). Tests **66 passing** core + 10 new in `tests/test_v13.py`. Most v1.3 features are opt-in: defaults match v1.2.2 behaviour at zero RAM cost.
...
|---------|-----------|----------------|-----------|---------|------|
| saltare | 46.82 MiB |      46.96 MiB | **46.96 MiB** |    1000 | 1109 |
| uvicorn | 49.07 MiB |      49.12 MiB | 49.12 MiB |    1000 | 1294 |
| granian | 57.72 MiB |      57.72 MiB | 57.72 MiB |    1000 | 1164 |

### Concurrent — 100 clients × 20 requests (2000 total)

| server  | idle RSS  | RSS after load | peak RSS  | reqs ok | rps  |
|---------|-----------|----------------|-----------|---------|------|
| saltare | 45.39 MiB |      45.76 MiB | **45.79 MiB** |    2000 | 1912 |
| uvicorn | 48.73 MiB |      49.77 MiB | 49.77 MiB |    2000 | 1887 |
| granian | 57.53 MiB |      57.53 MiB | 57.53 MiB |    2000 | 1880 |

### Idle keep-alive — 500 connections held open

| server  | idle RSS  | RSS after load | peak RSS  | reqs ok | conn rate |
|---------|-----------|----------------|-----------|---------|-----------|
| saltare | 45.37 MiB |      45.60 MiB | **45.60 MiB** |     500 | 1407      |
| uvicorn | 49.25 MiB |      54.63 MiB | 54.63 MiB |     500 | 1486      |
| granian | 56.62 MiB |      56.62 MiB | 56.62 MiB |     500 | 1358      |
...
|---------|----------|------------|---------------|-----------|--------------------|
|       1 |        — |  40.46 MiB |      0.00 MiB | 40.46 MiB |                 —  |
|       4 |        4 |  14.51 MiB |     39.98 MiB | 54.49 MiB |  161.84 MiB (−66%) |
...
- **Per-connection slope vs uvicorn**: 500 idle connections cost saltare **+0.23 MiB** (~470 B/conn) vs uvicorn's **+5.38 MiB** (~11 KiB/conn). That's a **~24× per-connection memory saving** vs uvicorn for a realistic workload (clients that hold connections open between bursts of activity). Granian adds essentially nothing (`+0.00 MiB`), so on per-conn slope saltare and granian are both comparable; saltare leads on the floor and on uvicorn-vs-anyone.
