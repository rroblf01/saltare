...
**Theme**: PEP 684 (per-interpreter GIL) foundation — moving multi-worker from pre-fork processes toward sub-interpreters for lower per-worker RAM. This release lands and verifies the gateway; the worker spawner + measurement follow.

Multi-worker today is pre-fork (`master.zig`): N OS processes, each a full CPython. fork shares code pages copy-on-write, but CPython's refcount writes dirty those shared pages, so resident memory diverges per worker. PEP 684 sub-interpreters (one process, N interpreters each with its own GIL on its own thread) keep the interpreter's immortal/static objects genuinely shared and avoid N process address spaces — the target saving is several MiB of Pss per worker. This is a multi-phase effort; v1.11 lands the multi-phase init, per-interpreter runtime state, a re-entrant event loop, and the per-interpreter-GIL declaration — verified by importing `_core` inside a real own-GIL interpreter.
...
The own-GIL import gateway is open and verified. What remains is the spawner that actually *runs* workers as sub-interpreters, and the measurement that justifies switching the default.

- **Interpreter-thread spawner** — for the `workers>1` path on Python 3.12+, spawn N threads, each `Py_NewInterpreterFromConfig({.gil = OWN})`, each importing the app and running `serveLoop()` on a shared `SO_REUSEPORT`/inherited listen fd, replacing the `fork()` in `module.zig`/`master.zig` (fork stays as the fallback and for Python < 3.12). Config is applied once in the main interpreter before spawning so the set-once invariant holds. Needs runtime validation with a real multi-interpreter serve harness (serve + request + clean shutdown across interpreters), not just import tests — the part that can't be proven by unit tests alone.
...
- `tests/test_subinterp.py` — proves `_core` imports in both a **shared-GIL** sub-interpreter (low-level `_interpreters` legacy) and an **own-GIL** sub-interpreter (PEP 734 `interpreters.create()`), the latter exercising the per-interpreter-GIL slot. Skips on source-only runs and where the sub-interpreter API isn't exposed. 362 passed, 6 skipped.
