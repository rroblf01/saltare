...
The module loads under a shared-GIL sub-interpreter, the racy event-loop state is isolated, and the loop is re-entrant — but per-interpreter-**GIL** (own-GIL) hosting is not wired up yet. saltare intentionally does **not** declare `Py_mod_multiple_interpreters = Py_MOD_PER_INTERPRETER_GIL_SUPPORTED` until the remaining items land — doing so earlier would be a false promise.

- **Per-interpreter module state (the gate)** — own-GIL interpreters require the module to declare it has no problematic process-global mutable state. saltare's set-once config globals (`g_limits`, `g_obs`, `g_timeouts`, TLS ctx, server line, log fd) violate that promise even though they're identical across workers. The clean fix is per-interpreter state via `m_size > 0` + `PyModule_GetState`, with config read through the module object instead of file-scope `var`s. This is the large remaining piece (it threads module state into `server.zig`'s read sites) and is the actual blocker for declaring the GIL slot.
- **Threading / event-loop model** — spawn N threads, each `Py_NewInterpreterFromConfig({.gil = OWN})`, each importing the app and running `serveLoop()` on a shared `SO_REUSEPORT`/inherited listen fd. Replaces the fork in `module.zig`/`master.zig` for the `workers>1` path (fork stays as the fallback and for Python < 3.12). Needs runtime validation with a real multi-interpreter serve harness, not just unit tests.
- **Declare support + the GIL slot** — add `Py_mod_multiple_interpreters` and `Py_mod_gil` slots (3.12+/3.13+, version-gated via `PY_VERSION_HEX`) once per-interpreter module state lands.
