...
- **Known limitation**: outbound flow control is not enforced — the server frames and sends the whole response, relying on TCP backpressure to bound buffering rather than waiting on the peer's `WINDOW_UPDATE`. The common cases work (verified to 200 KiB with the `h2` client), but a strict peer enforcing its receive window on a large response could `RST`. This is the next HTTP/2 refinement; it is a known edge, not a crash. Multiplexing is also still serial (one dispatch in flight per connection, as before).
...
- `tests/test_http2.py` — `TestHttp2RealClientConformance` drives the server with the real `h2` sans-IO client over TLS+ALPN and asserts valid HTTP/2 frames (HEADERS + DATA, not HTTP/1.1 bytes) for GET, POST-with-body, and a 60 KiB multi-frame response. `h2_response.zig` carries `zig test` unit tests that decode its output with the real HPACK decoder.
- Full suite: 367 passed, 6 skipped (CPython 3.14, native Zig 0.16 build).
